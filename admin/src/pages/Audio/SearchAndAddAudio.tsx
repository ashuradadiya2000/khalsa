...
import { addAudio, searchYoutubeAudio } from '../../services/audio'


interface ISearchAndAddAudioProp {
    refetch: () => void
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    videos: any
}

const SearchAndAddAudio: React.FC<ISearchAndAddAudioProp> = ({ refetch, videos }) => {
    ...
    const fetchVideos = async (query: string = '') => {
        setLoading(true)
        try {
            const { status, data } = await searchYoutubeAudio(query)
            if (status === 200) {
                setList(data.videos)
            }
    ...
    const handleAddToPlaylist = async () => {
        if (selected && selected.length) {
            try {
                ...
                const { status, data } = await addAudio(list)
                if (status === 201) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const ids = videos?.map((ele: any) => ele.videoId) || []
...
export default SearchAndAddAudio